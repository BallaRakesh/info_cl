...
from .MoCoTrain import get_moco_hidden_states
...
def new_old_train(
    ...
    # memory_repr, memory_labels = get_moco_hidden_states(model, args, data_collator, train_dataset, return_type='torch', return_labels=True)
    model.init_target(memory_repr, memory_labels)
    ...
    for epoch in range(num_train_epochs):
        ...
        for step, inputs in enumerate(train_dataloader):
            ...
            outputs = model(**inputs, new_old=True)
            # loss = outputs["loss"] if isinstance(outputs, dict) else outputs[0]
            # loss.backward()

            loss = freelb.attack(model, **inputs)
            # loss += args.new_old_lambda * outputs.new_old_loss
            new_old_loss = outputs.new_old_loss * args.new_old_lambda
            new_old_loss.backward()
            loss += new_old_loss.item()
    ...
    try:
        model.config.global_args = vars(model.config.global_args)
