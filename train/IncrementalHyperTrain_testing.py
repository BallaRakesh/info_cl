...
from configs.constants import use_custom_train_test_split, manual_task_plan, my_model, memory_path
from data import (
    FewRelData,
    TACREDData,
    MAVENData,
...
def incremental_hyper_train(
    ...
    # 4. Load model
    # -------------------------
    model = ModelForContinualLearning.from_pretrained(model_dir, config=config, ignore_mismatched_sizes=True)
    ...
    # Determine save directory (handle both file path and directory path)
    if os.path.isfile(previous_memory_path):
        memory_save_dir = os.path.dirname(previous_memory_path)
    elif os.path.isdir(previous_memory_path):
        memory_save_dir = previous_memory_path
    else:
        # Default to parent directory of model checkpoint
        memory_save_dir = os.path.dirname(model_checkpoint_path)

    os.makedirs(memory_save_dir, exist_ok=True)

    save_path = os.path.join(memory_save_dir, f"memory_data_{new_task_id}.pkl")
    logger.info(f"Saving updated memory to: {save_path}")
    with open(save_path, "wb") as f:
        pickle.dump(memory_data, f)

    repr_save_path = os.path.join(memory_save_dir, f"memory_repr_labels_{new_task_id}.pt")
    ...
    # Save final model
    final_model_path = os.path.join(memory_save_dir, f"model_after_task_{new_task_id}")
