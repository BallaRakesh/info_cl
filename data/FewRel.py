...
class FewRelData(BaseData):
    ...
    def preprocess(self, raw_data, tokenizer):
        ...
        return res
    def preprocess_del(self, raw_data, tokenizer):
        subject_start_marker = tokenizer.convert_tokens_to_ids(self.entity_markers[0])
        object_start_marker = tokenizer.convert_tokens_to_ids(self.entity_markers[2])
        res = []
        result = tokenizer(raw_data['sentence'])
        ...
        for idx in range(len(raw_data['sentence'])):
            try:
                ...
                print(f"Error in sentence {idx}: {raw_data['sentence'][idx]}")
                print(f"Input IDs: {result['input_ids'][idx]}")
                print(f"Looking for subject marker: {subject_start_marker}, object marker: {object_start_marker}")
                print(f"Entity markers: {self.entity_markers}")
                # raise e
