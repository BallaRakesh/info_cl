...
def compute_forgetting_rate(detail_metrics, task_seq, id2label, mode='task'):
    ...
    if mode == 'label':
        ...
        task_fr_rec = []
        for task in task_seq[:-1]:  # skip last task since no future task to measure forgetting
            # Collect metrics for all labels in this task
            cur_task_metric = []
            for label in task:
                label_name = id2label[label]
                metric_values = np.array(label_metric_rec[label_name], dtype=object)
                # Convert list to float array if possible
                if isinstance(metric_values[0], (list, np.ndarray)):
                    metric_values = np.array(metric_values, dtype=float).flatten()
            
                cur_task_metric.append(metric_values)

            # --- Handle unequal metric lengths ---
            min_len = min(len(m) for m in cur_task_metric)
            cur_task_metric = np.array([m[:min_len] for m in cur_task_metric], dtype=float)

            # Average metrics across labels
            cur_task_metric = np.mean(cur_task_metric, axis=0)

            # Forgetting rate for current task
            cur_task_fr = np.max(cur_task_metric) - cur_task_metric[-1]
            task_fr_rec.append(cur_task_fr)
        # Return mean forgetting rate across all tasks
        return np.mean(task_fr_rec)
